...
사용자 피드백:
  - object_001 (빨강): cam1/cam2에서 위아래 뒤집혀있음 → flip 후보 탐색
  - object_002 (노랑): cam2 반시계 회전 → yaw 자유도 확대
  - object_003 (곤색): 가로 길이 부족 → anisotropic scale 허용
  - object_004 (민트): cam1 오른쪽 이동 → translation 자유도 확대

변경사항:
  1. 각 non-symmetric 물체(001, 003)에 대해 {원본, h1-flip, h2-flip} × Nelder-Mead
  2. Anisotropic scale 탐색 (3축 독립 log-scale)
  3. 더 넓은 simplex (translation 4cm, yaw 15°)

저장: src/output/pose_fitted/v2_{timestamp}_frame_{id}/
"""
...
def fit_pose(pose_json, sam_masks, frames, glb_path: Path, obj_name: str,
             max_iter: int = 300, table_n: np.ndarray = None):
    ...
    for cam, m in zip(frames, sam_masks):
        if m is None or (m > 0).sum() < 100:
            continue
        c, e = sam_centroid_3d(m, cam)
        if c is not None:
            cents.append(c); exts.append(e)
    ...
    flip_candidates = [("original", np.eye(3))]
    if not is_sym:
        # Flip around horizontal axes (사용자 지적: red arch 뒤집힘)
        up = -table_n if table_n is not None else np.array([0, -1, 0])
        up = up / (np.linalg.norm(up) + 1e-12)
        # perpendicular horizontal axes
        ref = np.array([1, 0, 0]) if abs(up[0]) < 0.9 else np.array([0, 1, 0])
        h1 = np.cross(up, ref); h1 /= np.linalg.norm(h1) + 1e-12
        h2 = np.cross(up, h1)
        for axis, tag in [(h1, "flip_h1"), (h2, "flip_h2")]:
            R_flip = Rot.from_rotvec(axis * np.pi).as_matrix()
            flip_candidates.append((tag, R_flip))
    ...
    def loss(params, R_init):
        T, scale_axis = apply(params, R_init)
        iou_sum = 0; n = 0
    ...
    init_simplex[4][3] = np.radians(15)  # yaw
    init_simplex[5][4] = np.radians(8)
    init_simplex[6][5] = np.radians(8)
    init_simplex[7][6] = np.log(1.15)  # scale x
    init_simplex[8][7] = np.log(1.15)  # scale y
    init_simplex[9][8] = np.log(1.15)  # scale z

    best = None
    for tag, R_init in flip_candidates:
        x0 = np.zeros(n_dof)
        l0 = loss(x0, R_init)
        print(f"  [{tag}] initial loss={l0:.4f}")
        res = minimize(lambda p: loss(p, R_init), x0, method="Nelder-Mead",
                       options={"maxiter": max_iter, "xatol": 1e-4, "fatol": 1e-4,
                                "initial_simplex": init_simplex, "disp": False})
        print(f"  [{tag}] final loss={res.fun:.4f} IoU={1-res.fun:.3f}")
        if best is None or res.fun < best["loss"]:
            best = {"tag": tag, "R_init": R_init, "res": res, "loss": res.fun}

    print(f"  => best: {best['tag']} IoU={1-best['loss']:.3f}")

    T_new, scale_axis = apply(best["res"].x, best["R_init"])
    ...
    new_pose["flip_selected"] = best["tag"]
    return new_pose
...
def main():
    ...
    out_dir = Path("src/output/pose_fitted") / f"v2_{ts}_frame_{args.frame_id}"
