...
사용자 피드백:
  - cam2에서 노랑/민트 실린더 여전히 차이
  - 빨강 물체도 cam2에서 수평 회전값 적용 필요

변경사항 vs v3:
  1. cam2 weight 2.0배 (cam0/cam1 = 1.0)
  2. 실린더: yaw 0/90/180/270 multi-start 탐색
  3. 비대칭 물체: 24 orientation × 4 yaw pre-rotation 조합 전수 → fine
  4. Initial simplex yaw 범위 ±20° 확장

저장: src/output/pose_fitted/v4_{timestamp}_frame_{id}/
"""
...
def gen_orientation_candidates(is_sym: bool):
    if is_sym:
        # yaw symmetric: 원본 + 4개 yaw pre-rotation
        out = [("original", np.eye(3))]
        for yaw in [90, 180, 270]:
            R = Rot.from_euler("Y", yaw, degrees=True).as_matrix()
            out.append((f"yaw_{yaw}", R))
        return out
    angles = [0, 90, 180, 270]
    out = {}
...
def fit_pose(pose_json, sam_masks, frames, glb_path: Path, obj_name: str,
    ...
    ori_cands = gen_orientation_candidates(is_sym)
    print(f"  orientations: {len(ori_cands)}, cam weights: {CAM_WEIGHTS}")
    ...
    def apply_fn(params, R_init):
        if is_sym:
            dx, dy, dz, dyaw, dpitch, droll, dls_r, dls_z = params
            sr = s0 * float(np.exp(np.clip(dls_r, np.log(0.70), np.log(1.40))))
            sz_ = s0 * float(np.exp(np.clip(dls_z, np.log(0.70), np.log(1.40))))
            scale_axis = np.array([sr, sr, sz_])
        else:
    ...
    def loss(params, R_init):
        ...
        for ci, (cam, obs) in enumerate(zip(frames, obs_small)):
            ...
            rnd = np.zeros((h, w_img), dtype=np.uint8)
    ...
    # 1단계: 모든 orientation 빠른 평가
    quick_iter = 50 if len(ori_cands) > 4 else max_iter
    scored = []
    for tag, R_init in ori_cands:
    ...
    print(f"  top{top_n} (weighted): " +
          ", ".join(f"{tag}={1-l:.3f}" for l, tag, _, _ in scored[:top_n]))

    # 2단계: top 후보 fine refinement
    best = None
    ...
    for ci, (cam, obs) in enumerate(zip(frames, obs_small)):
        if obs is None:
            per_cam_iou.append(None); continue
        V = np.asarray(mesh.vertices, dtype=np.float64)
        V_obj = (V - model_center) * scale_axis
        Vh = np.hstack([V_obj, np.ones((len(V_obj), 1))])
        V_base = (T_new @ Vh.T)[:3].T
        T_cb = np.linalg.inv(cam.T_base_cam)
        V_cam = (T_cb @ np.hstack([V_base, np.ones((len(V_base), 1))]).T)[:3].T
        z = V_cam[:, 2]; ok = z > 0.05
        K = cam.intrinsics.K
        ...
        inter = int(np.logical_and(rnd_b, obs).sum())
        union = int(np.logical_or(rnd_b, obs).sum())
        per_cam_iou.append(inter / max(union, 1))

    print(f"  => best: {best['tag']} weighted_loss={best['loss']:.3f} "
          f"per-cam IoU: {[f'{x:.3f}' if x is not None else 'N/A' for x in per_cam_iou]}")
    ...
    new_pose["per_cam_iou"] = per_cam_iou
    return new_pose
...
def main():
    ...
    ap.add_argument("--pose_dir",
                    default="src/output/pose_fitted/v3_20260420_171025_frame_000000")
    ...
    out_dir = Path("src/output/pose_fitted") / f"v4_{ts}_frame_{args.frame_id}"
