...
# hue 반경 / saturation 하한 / value 구간 (주 검출용 — 타이트하게 유지)
# cam0 등에서 마스크가 비는 경우는 mask_refine_with_depth_cluster의
# cross-camera 투영-기반 복원 경로로 채움.
COLOR_THRESHOLDS = {
...
def mask_refine_with_depth_cluster(
    ...
    ref_h = COLOR_REF_HSV[object_name][0] if object_name in COLOR_REF_HSV else None
    # 마스크 복원 시 더 느슨한 HSV 임계값 (재투영이 이미 위치를 제약함)
    recover_thr = {"hue": 22.0, "s_min": 25, "v_min": 15, "v_max": 255}
    for cam, mask in zip(frames, masks):
        h, w = cam.intrinsics.height, cam.intrinsics.width
        ...
        if int(np.count_nonzero(mask)) < 50 and ref_h is not None:
            # 복원 모드: 원본 마스크가 없으므로 투영 영역 안에서 느슨한
            # HSV 색상 재검증으로 물체 픽셀을 찾음. 3D 클러스터 투영이
            # 위치 제약을 주므로 안전 캡 완화 (5%→15%, 3%→10%).
            proj_dilated = cv2.dilate(proj_mask, k_big, iterations=2)
            proj_dilated = cv2.morphologyEx(proj_dilated, cv2.MORPH_CLOSE, k_big)
            area_px = int(np.count_nonzero(proj_dilated))
            if area_px > int(h * w * 0.15):
                refined.append(mask)
                continue
            hsv = cv2.cvtColor(cam.color_bgr, cv2.COLOR_BGR2HSV)
            h_img = hsv[:, :, 0].astype(np.float32)
            s_img = hsv[:, :, 1]; v_img = hsv[:, :, 2]
            hue_ok = hsv_circular_distance(h_img, float(ref_h)) <= recover_thr["hue"]
            sv_ok = ((s_img >= recover_thr["s_min"])
                     & (v_img >= recover_thr["v_min"])
                     & (v_img <= recover_thr["v_max"]))
            color_ok = (hue_ok & sv_ok).astype(np.uint8) * 255
            recovered = cv2.bitwise_and(color_ok, proj_dilated)
            recovered = cv2.morphologyEx(recovered, cv2.MORPH_OPEN, k)
            recovered = cv2.morphologyEx(recovered, cv2.MORPH_CLOSE, k)
            if int(np.count_nonzero(recovered)) > int(h * w * 0.10):
                refined.append(mask)
                continue
            # 최소 픽셀 기준 확인: 너무 적으면 복원 포기 (빈 마스크 유지)
            if int(np.count_nonzero(recovered)) < 150:
        ...
        refined.append(cv2.bitwise_and(mask, proj_mask))
    return refined
...
def render_and_compare_refine(
    ...
    simplify_faces: int = 0,      # 0=간소화 OFF (원본 mesh)
        ...
        if L < step["best"] - 1e-4:
            ...
            if verbose:
                ...
                if pitch_roll_dof:
                    ...
                    extra = ""
                    scale_idx = 4
                print(f"        [iter {step['n']:3d}] IoU={1-L:.3f} "
                      f"dx={params[0]*100:+.1f}cm dy={params[1]*100:+.1f}cm "
                      f"dz={params[2]*100:+.1f}cm dyaw={np.degrees(params[3]):+.1f}°"
    ...
    if verbose:
        print(f"      [render-refine] mean_IoU {1-loss0:.3f}→{1-res.fun:.3f} "
...
def render_and_compare_with_flips(
    ...
    for axis, tag in [(h1, "flip_h1"), (h2, "flip_h2")]:
        ...
        flipped = PoseEstimate(
            ...
            quaternion_xyzw=rot_tmp.as_quat(),
            euler_xyz_deg=rot_tmp.as_euler("xyz", degrees=True),
            scale=pose.scale, confidence=pose.confidence,
        ...
        base_candidates.append((tag, flipped))
    ...
    for tag, p in base_candidates:
        for yaw_deg in yaw_offsets:
            if yaw_deg == 0.0:
                ...
                continue
            R_yaw = Rot.from_rotvec(target_up * np.radians(yaw_deg)).as_matrix()
            T_new = p.T_base_obj.copy()
            T_new[:3, :3] = R_yaw @ p.T_base_obj[:3, :3]
            rot_tmp = Rot.from_matrix(T_new[:3, :3])
            p_rot = PoseEstimate(
                T_base_obj=T_new,
                position_m=T_new[:3, 3].copy(),
                quaternion_xyzw=rot_tmp.as_quat(),
                euler_xyz_deg=rot_tmp.as_euler("xyz", degrees=True),
                scale=p.scale, confidence=p.confidence,
                fitness=p.fitness, rmse=p.rmse,
                depth_score=p.depth_score, coverage=p.coverage,
                silhouette_score=p.silhouette_score,
            )
            all_candidates.append((f"{tag}_yaw{int(yaw_deg)}", p_rot))
    ...
    for i, (tag, cand) in enumerate(all_candidates):
        print(f"      [flip-search] refining {i+1}/{len(all_candidates)} ({tag})")
        try:
    ...
    results.sort(key=lambda x: x[1].silhouette_score, reverse=True)
    best_tag, best = results[0]
    summary = ", ".join(f"{t}={p.silhouette_score:.3f}" for t, p in results[:5])
    print(f"      [flip-search] best={best_tag} IoU={best.silhouette_score:.3f} "
          f"(top5: {summary})")
    return best
...
def refine_masks_with_pose(
    ...
    for cam, mcol in zip(frames, masks_color):
        ...
        if int(np.count_nonzero(mcol)) >= 100:
            refined.append(cv2.bitwise_and(mcol, silh_d))
        else:
...
def estimate_one(
    ...
    if depth_seg_masks is not None:
        masks_raw = [m.copy() for m in depth_seg_masks]
    else:
        masks_raw = []
        for ci, cam in enumerate(frames):
            raw = color_mask_for_object(cam.color_bgr, object_name)
            best = best_connected_component(raw)
            masks_raw.append(best)

    # Area outlier drop: 색상 임계가 특정 카메라(cam0)에 안 맞아 다른 물체의
    # 노이즈가 주 blob으로 선택되면 면적이 극단적으로 커짐 → 3D 융합을 오염.
    # 중앙값 대비 >6× 크거나 <0.12× 작은 마스크는 신뢰 불가로 드랍.
    areas = [int(np.count_nonzero(m)) for m in masks_raw]
    print(f"      [raw_mask] areas cam0={areas[0]} cam1={areas[1]} cam2={areas[2]}")
    valid_areas = [a for a in areas if a >= 100]
    if len(valid_areas) >= 3:
        med = float(np.median(valid_areas))
        # 타이트한 outlier 필터: 중앙값의 2.5배보다 크거나 0.33배보다 작으면 drop
        for ci, a in enumerate(areas):
            if a < 100:
                continue
            if a > med * 2.5 or a < med * 0.33:
                print(f"      [mask-drop] cam{ci} area={a} median={med:.0f} "
                      f"→ outlier, drop")
                masks_raw[ci] = np.zeros_like(masks_raw[ci])
    ...
    for ci in range(3):
        if int(np.count_nonzero(masks[ci])) < 50:
            masks[ci] = masks_raw[ci]  # 정제 후 너무 작아지면 원본 사용
...
def run(frame_id: str, only: Optional[List[str]] = None,
    ...
    for name in object_names:
        ...
        ds_masks = depth_seg_all.get(name) if depth_seg_all else None
